...
from funcao import valida_senha, enviando_email, gerar_token, remover_bearer, senha_secreta
...
def login():
    ...
    cur = con.cursor()
    cur.execute("SELECT u.EMAIL , u.SENHA, u.id_usuario  FROM USUARIO u WHERE u.EMAIL = 	?", (email,))
    usuario = cur.fetchone()
    if usuario:
        if check_password_hash(usuario[1], senha):
            token = gerar_token(usuario[2])

            resp = make_response(jsonify({'mensagem': 'Login com sucesso'}), 200)
